...
export class ApiKeyService {
    ...
    async validateApiKeyWithRole(apiKey: string, requiredRole: string): Promise<boolean> {
        // Validate API key based on required role
        const apiKeyRecord = await this.apiKeyRepository.findOne({
            where: { key: apiKey, role: requiredRole },  // Validate the API key with the required role
        });
    
        return !!apiKeyRecord;  // Return true if the API key is valid, false otherwise
    }
